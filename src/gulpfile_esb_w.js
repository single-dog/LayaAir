...
const esbuild = require("./publishTool/rollup-plugin-esbuild/dist");


let green = '#22c382';
let gray = '#969696';
let red = '#e34848';
let defaultColor = '#dadada';

function takeTime(start, f = null) {
    let dt = Date.now() - start;
    if (f) {
        return `${dt / (f == 's' ? 1000 : 1)} ${f}`;
    } else {
        return dt > 999 ? `${dt / 1000} s` : `${dt} ms`;
    }
}

function showBuildErrorLog(event ) {
    let errors = event.error.errors;
    if (errors) {
        let workSpaceDir = Editor.projectPath;
        for (let i = 0, len = errors.length; i < len; i++) {
            let error = errors[i];
            let localtion1 = error.location;
            console.error(`[color=${defaultColor}][color=${red}] Error [/color] [color=${gray}] in [/color] [color=${defaultColor}][b]${localtion1.file.replace(workSpaceDir, ".")}[/b][/color][color=${green}] ${localtion1.line}:${localtion1.column} [/color]
            [color= ${defaultColor}][b]${localtion1.lineText}[/b][/color][/color]
            [color= ${defaultColor}]${error.text}[/color]`);
        }
    } else {
...
function watch() {
    ...
    for (let i = 0; i < 1; ++i) {
        let mChokidarOptions = {

        };
        let workSpaceDir = '';// packsDef[i].input;
        let esbuildPlugin = esbuild({
            include: /.*(.ts|.d.ts|.tsx)$/,
        ...
        const watchOptions = {
            ...
            watch: {
                chokidar: mChokidarOptions,
            },
            treeshake: false,
            cache: true,
            output: packsDef[i].libName == "core" ? {
                file: packsDef[i].out,
                format: 'iife',
        ...
        watcher.on("change", (id , event ) => {
            ...
            console.log(`[color=${defaultColor}]${event.event}: ${id.replace(workSpaceDir, ".")} [/color]`);
        });
        let startT = Date.now();
        watcher.on('event', (event ) => { 
            ...
            if (event.code == 'BUNDLE_END') { 
                ...
                console.log(`${packsDef[i].out} compiled successfully`); 
            
            } else if (event.code == "ERROR") { 
                showBuildErrorLog(event);
...
gulp.task('buildJS22', async function () {
    for (let i = 0; i < packsDef.length; ++i) {
        const subTask = await rollup.rollup({
            input: packsDef[i].input,
            output: {
